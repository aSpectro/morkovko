    ...
    try {
      ...
      if (fund) {
        ...
        embed.setDescription(
          ...
          Размер призового фонда **${abbreviateNumber(fund.fundSize)}** ${locale.getCurrency()}.\n
          ...
          4. По истечению вопросов, игрок, набравший больше всех баллов получает ${locale.getEnum('морковки')} из призового фонда.\n
    ...
    if (maxPoints === 0) {
      ...
      if (winners.length > 1) {
        ...
        embed.setDescription(
          ...
          Их приз: **${abbreviateNumber(prize)}** ${locale.getCurrency()} каждому!`,
        );
      } else {
        embed.setDescription(
          `**Встречайте победителя викторины!**\n
          🏆 <@${winners[0].id}> набрал больше всех баллов - **${maxPoints}**!\n
          Его приз: **${abbreviateNumber(prize)}** ${locale.getCurrency()}!`,
        );
      }
