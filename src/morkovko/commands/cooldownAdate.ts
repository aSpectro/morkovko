      ...
      service.checkUser(user.id).then((res) => {
        if (res.status === 200 && res.player) {
          ...
          if (count && player.points >= price * count && this.canBuy(player.carrotSize, 'cooldowns', player.config.cooldowns.adate, count)) {
            ...
            } else if (!this.canBuy(player.carrotSize, 'cooldowns', player.config.cooldowns.adate, count)) {
              const acceptedCount = 50 - (player.config.cooldowns.adate + count);
              if (acceptedCount <= 0) {
                this.embed.setDescription(
                  `Ты не можешь купить этот бонус! Сейчас у тебя ${player.config.cooldowns.adate}/50.`,
                );
                ...
                this.embed.setDescription(
                  `Ты можешь купить не больше ${acceptedCount} бонусов! Сейчас у тебя ${player.config.cooldowns.adate}/50.`,
                );
              }
            } else {
              this.embed.setDescription(
